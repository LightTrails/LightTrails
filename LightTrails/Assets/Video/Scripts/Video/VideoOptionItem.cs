...
public class VideoOptionItem : OptionItem
{
    private VideoFileInfo _videoFileInfo;
    private DeleteModeVideoToggleButton _deleteToggleButton;

    void Start()
    {
        Initialize("Garbage");
        _deleteToggleButton = FindObjectOfType<DeleteModeVideoToggleButton>();
        //transform.Find("Delete").GetComponent<Button>().onClick.AddListener(Delete);
    }

    void Update()
    {
        var deleteMode = _deleteToggleButton.DeleteMode;
    ...
    private void SetProject(bool active)
    {
    ...
    internal void SetVideoInfo(VideoFileInfo videoFileInfo)
    {
        ...
        transform.Find("Content/Type/Value").GetComponent<Text>().text = videoFileInfo.Type.Replace(".", "").ToLowerInvariant();
        transform.Find("Content/Size/Value").GetComponent<Text>().text = SizeSuffix(videoFileInfo.Length);
    ...
    {
        if (_deleteToggleButton.DeleteMode)
        {
            Delete();
            FadeoutByResize();
        }
